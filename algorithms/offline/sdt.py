...
def eval_rollout(
    ...
    for step in range(model.episode_len):
        ...
        predicted_actions, predicted_subgoal = model(  # fix this noqa!!!
        ...
        if step % subgoal_interval == 0 or next_state == current_subgoal:
            if next_state == current_subgoal:
...
def train(config: TrainConfig):
    ...
    for step in trange(config.update_steps, desc="Training"):
        ...
        subgoal_loss = F.mse_loss(predicted_subgoal,
                                  sg_states.detach(),
                                  reduction="none")
        lambda_t = max(0.1, 1.0 - (step / config.update_steps))  # Decay lambda from 1 to 0.1 over time
        # [batch_size, seq_len, action_dim] * [batch_size, seq_len, 1]
        loss = action_loss.mean() + (lambda_t) * subgoal_loss.mean()
        # loss = subgoal_loss.mean()
