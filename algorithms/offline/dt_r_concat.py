...
class DecisionTransformer(nn.Module):
    def __init__(
        ...
        self.state_emb = nn.Linear(state_dim + 1, embedding_dim)
    ...
    def forward(
        ...
        state_with_returns = torch.cat((states, returns.unsqueeze(-1)), dim=-1)  # [batch_size, seq_len, state_dim+1]

        batch_size, seq_len = states.shape[0], states.shape[1]
        time_emb = self.timestep_emb(time_steps)
        state_emb = self.state_emb(state_with_returns) + time_emb

        sequence = (
            torch.stack([state_emb,], dim=1)
            .permute(0, 2, 1, 3)
            .reshape(batch_size, 1 * seq_len, self.embedding_dim)
        )
        if padding_mask is not None:
            # [batch_size, seq_len * 3], stack mask identically to fit the sequence
            padding_mask = (
                torch.stack([padding_mask,], dim=1)
                .permute(0, 2, 1)
