...
class TrainConfig:
    ...
    alpha: float = 1
    # evaluation params
    target_returns: Tuple[float, ...] = (5000.0, 2500.0)
...
def train(config: TrainConfig):
    ...
    for step in trange(config.update_steps, desc="Training"):
        ...
        action_loss = F.mse_loss(predicted_actions,
                                 actions.detach(),
                                 reduction="none",)
...
#        loss = (alpha)*action_loss.mean() + (1 - alpha)*subgoal_loss.mean()
        loss = (0.5)*action_loss.mean() + (0.5)*subgoal_loss.mean()
        ...
        if step % config.eval_every == 11 or step == config.update_steps - 1:
            ts_eval = time.time()
            model.eval()
