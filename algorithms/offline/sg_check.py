...
class SequenceDataset(IterableDataset):
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        return states, actions, returns, sg_states, countdown_array, time_steps, mask
...
class GoalTransformer(nn.Module):
    ...
    def forward(
        ...
        sequence = (
            torch.stack([returns_emb, state_emb], dim=1)
            .permute(0, 2, 1, 3)
...
def train(config: TrainConfig):
    ...
    for i in range(config.seeds):
        ...
        for step in trange(30000, desc="Training"):
            ...
            states, actions, returns, sg_states, countdown, time_steps, mask = [b.to(config.device) for b in batch]
            padding_mask = ~mask.to(torch.bool)
            predicted_sub = goal_model(
                states=states,
                # returns_to_go=returns,
                # subgoals=sg_states,
                returns_to_go=countdown,
