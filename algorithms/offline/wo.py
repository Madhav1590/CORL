...
class SequenceDataset(IterableDataset):
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        # slices = compute_subgoals(rewards)
        # print(len(rewards))
...
class DecisionTransformer(nn.Module):
    ...
    def forward(
        ...
        action_out = self.action_head(out[:, 1::4]) * self.max_action
        subgoal_out = self.subgoal_head(out[:, 1::4])
...
def eval_rollout(
    ...
    _, predicted_subgoal = model(
        states[:, : 1][:, -model.seq_len:],
        actions[:, : 1][:, -model.seq_len:],
        returns[:, : 1][:, -model.seq_len:],
        sg_states[:, : 1][:, -model.seq_len:],  # include subgoal states
        time_steps[:, : 1][:, -model.seq_len:],
    )
    # print(predicted_subgoal.shape)
    # print(predicted_subgoal)
    ...
    for step in range(model.episode_len):
        ...
        predicted_actions, predicted_subgoal = model(
            ...
            sg_states[:, : step + 1][:, -model.seq_len:],  # include subgoal states
        ...
        if (step+1) % model.seq_len == 0:
            current_subgoal = predicted_subgoal
            slices = torch.as_tensor([-1] * model.seq_len, dtype=torch.long)
            sg_states[:, : step + 1][:, -model.seq_len:] = states[:, : step + 1][:, slices]

        sg_states[:, step + 1] = torch.as_tensor(current_subgoal)
...
#         num_episodes=100
# ) -> dict:
#     # Statistics dictionary to track across all episodes
#     stats = {
#         "total_return": [],
#         "total_length": [],
...
#         current_subgoal = None
#         for step in range(model.episode_len):
#             # Get predicted action and subgoal from the model
#             predicted_actions, predicted_subgoal = model(
#                 states[:, : step + 1][:, -model.seq_len:],
#                 actions[:, : step + 1][:, -model.seq_len:],
#                 returns[:, : step + 1][:, -model.seq_len:],
...
#             returns[:, step + 1] = torch.as_tensor(returns[:, step] - reward)
#
#             # Update sg_states with the predicted subgoal for the current interval
#             sg_states[:, step + 1] = torch.as_tensor(predicted_subgoal)
#
#             # Every `subgoal_interval`, check if subgoal was achieved
#             if (step + 1) % subgoal_interval == 0:
#                 stats["subgoals_attempted"] += 1
#
#                 # Compute subgoals for the past interval
#                 past_rewards = rewards[:, step + 1 - subgoal_interval: step + 1]
#                 subgoal_indices = compute_subgoals(past_rewards.cpu().numpy(), subgoal_interval=subgoal_interval)
#
#                 # Check how far the achieved state was from the subgoal
#                 achieved_state = states[:, step]  # State at the current step
#                 target_subgoal_state = states[:, subgoal_indices[-1]]  # Target subgoal state
#                 distance = F.mse_loss(achieved_state, target_subgoal_state).item()
#
#                 # Record the distance
#                 stats["subgoal_distances"].append(distance)
#
#                 # Check if the subgoal was achieved (e.g., based on a threshold for closeness)
#                 if distance < 1e-3:  # You can adjust this threshold
#                     stats["subgoals_achieved"] += 1
#
#                 # Update sg_states for the past interval with actual subgoals from the trajectory
#                 for t in range(step + 1 - subgoal_interval, step + 1):
#                     sg_states[:, t + 1] = states[:, subgoal_indices[t - (step + 1 - subgoal_interval)]]
#
#             episode_return += reward
#             episode_len += 1
#
...
#
#         # Store episode stats
#         stats["total_return"].append(episode_return)
#         stats["total_length"].append(episode_len)
#
#     # Aggregate statistics over all episodes
#     stats["avg_return"] = np.mean(stats["total_return"])
#     stats["avg_length"] = np.mean(stats["total_length"])
#     stats["avg_subgoal_distance"] = np.mean(stats["subgoal_distances"]) if stats["subgoal_distances"] else None
#     stats["subgoal_success_rate"] = stats["subgoals_achieved"] / stats["subgoals_attempted"] if stats[
#                                                                                                     "subgoals_attempted"] > 0 else 0
#
#     return stats
...
def train(config: TrainConfig):
    ...
    for step in trange(config.update_steps, desc="Training"):
        ...
        subgoal_loss = F.mse_loss(predicted_subgoal,
                                  sg_states.detach(),
                                  reduction="none")
        # [batch_size, seq_len, action_dim] * [batchd_size, seq_len, 1]
        alpha = 0.8
        loss = action_loss.mean() + 0.5 * subgoal_loss.mean()
