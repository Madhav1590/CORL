...
class SequenceDataset(IterableDataset):
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        return states, actions, returns, sg_states, time_steps, mask #,countdown_array
...
class GoalTransformer(nn.Module):
    def __init__(
        ...
        self.return_state_emb = nn.Linear(state_dim + 1, embedding_dim)
    ...
    def forward(
        ...
        return_state = torch.cat([returns_to_go.unsqueeze(-1), states], -1)
        return_state_emb = self.return_state_emb(return_state) + time_emb

        sequence = (
            torch.stack([return_state_emb], dim=1)
            .permute(0, 2, 1, 3)
        ...
        subgoal_out = self.subgoal_head(out[:, 1::2])
...
class DecisionTransformer(nn.Module):
    def __init__(
        ...
        self.dist_emb = nn.Linear(1, embedding_dim)
    ...
    def forward(
        ...
        dist_emb = self.dist_emb(distances.unsqueeze(-1)) + time_emb
        ...
        out = self.out_norm(out)
        action_out = self.action_head(out[:, 1::5])# + out[:, 3::4])
        out = (action_out, None)
        return out



def is_subgoal_reached(state: np.ndarray, subgoal: np.ndarray, threshold: float = 0.1) -> bool:
...
def eval_rollout(
    ...
    countdown = torch.zeros(1, model.episode_len + 1, dtype=torch.float, device=device)
    ...
    countdown[:, 0] = torch.as_tensor(config.subgoal_interval, device=device)
    ...
    current_subgoal = goal_model(
        states=states[:, :1],
        returns_to_go=countdown[:, :1],
        time_steps=time_steps[:, :1],
    )[0, -1].cpu().numpy()

    sg_states[:, 0] = torch.as_tensor(current_subgoal, device=device)
    subgoal_achieved = 0
    ...
    for step in range(model.episode_len):
        predicted_actions, _ = model(
            ...
            countdown[:, : step + 1][:, -model.seq_len:],
        ...
        if state_reached_subgoal or subgoal_step_count >= config.subgoal_interval:
            current_subgoal = goal_model(
                ...
                returns_to_go=countdown[:, : step + 1][:, -model.seq_len:],
                time_steps=time_steps[:, : step + 1][:, -model.seq_len:],
            )[0, -1].cpu().numpy()

            subgoal_step_count = 0
            countdown[:, step + 1] = torch.as_tensor(config.subgoal_interval, device=device)

        else:
            countdown[:, step + 1] = torch.as_tensor(countdown[:, step] - 1, device=device)
...
def train(config: TrainConfig):
    ...
    for i in range(config.seeds):
        ...
        for step in trange(30000, desc="Training"):
        ...
        for step in trange(config.update_steps, desc="Training"):
            ...
            subgoal_distance_loss = (np.linalg.norm(sg_states.cpu().detach() - states.cpu().detach()) * mask.unsqueeze(-1)).mean()
            sg_dist_loss.append(subgoal_distance_loss)
            action_loss = F.mse_loss(predicted_actions, actions.detach(), reduction="none")
            action_loss = (action_loss * mask.unsqueeze(-1)).mean()
            # loss = action_loss + (0.5) * subgoal_distance_loss
            loss = action_loss
            ...
            if (step % config.eval_every == 0 or step == config.update_steps - 1) and step != 0:
                ...
                print(f"Seed: {train_seed}, Average Episode Length: {np.mean(eval_lens)}")
                model.train()
