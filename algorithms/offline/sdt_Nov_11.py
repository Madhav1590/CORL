...
class GoalTransformer(nn.Module):
    ...
    def forward(
        ...
        subgoal_out = self.subgoal_head(torch.cat((out[:, 1::3], out[:, 0::3]), dim=-1))
...
class DecisionTransformer(nn.Module):
    ...
    def forward(
        ...
        action_out = self.action_head(out[:, 3::4])#torch.cat((out[:, 3::4], out[:, 1::4]), dim=-1))# + out[:, 3::4])
...
def eval_rollout(
    ...
    for step in range(model.episode_len):
        ...
        state_reached_subgoal = is_subgoal_reached(states[:, step + 1].cpu().numpy(), current_subgoal)
        sg_count += state_reached_subgoal
    ...
    return episode_return, episode_len, sg_count
...
def train(config: TrainConfig):
    ...
    for step in trange(config.update_steps, desc="Training"):
        ...
        act_loss.append(loss.cpu().detach().numpy())
        ...
        # if step == 999:
            ...
            for target_return in config.target_returns:
                ...
                for _ in trange(config.eval_episodes, desc="Evaluation", leave=False):
                    eval_return, eval_len, sg_count = eval_rollout(
                        model=model,
                ...
                print(f'Avg Subgoal achieved: {np.mean(sg_counts)}')
                wandb.log(
                    {
