...
class DecisionTransformer(nn.Module):
    ...
    def forward(
        ...
        state_with_rewards = torch.cat((states, returns_to_go.unsqueeze(-1)), dim=-1)  # [batch_size, seq_len, state_dim+1]

        batch_size, seq_len = states.shape[0], states.shape[1]
        # [batch_size, seq_len, emb_dim]
        time_emb = self.timestep_emb(time_steps)
        state_emb = self.state_emb(state_with_rewards) + time_emb
