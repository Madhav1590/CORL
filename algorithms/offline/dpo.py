...
def load_d4rl_trajectories(
    ...
    info = {
        ...
        "action_mean": dataset["actions"].mean(0, keepdims=True),
        "action_std": dataset["actions"].std(0, keepdims=True) + 1e-6,
...
def calculate_DPO_loss(train_logits_pos, train_logits_neg, ref_logits_pos, ref_logits_neg, info, beta=0.1):
    # Compute logits difference (preference margins)
    train_prob_pos = calculate_joint_probability(train_logits_pos.cpu(), info)
    train_prob_neg = calculate_joint_probability(train_logits_neg.cpu(), info)
    delta_train = train_prob_pos - train_prob_neg

    ref_prob_pos = calculate_joint_probability(ref_logits_pos.cpu(), info)
    ref_prob_neg = calculate_joint_probability(ref_logits_neg.cpu(), info)
    delta_ref = ref_prob_pos - ref_prob_neg

    preference_margin = beta * (delta_train - delta_ref)
    loss = -F.logsigmoid(torch.as_tensor(preference_margin))
    return loss



def calculate_joint_probability(actions, info):
    mu = info['action_mean']
    sigma = info['action_std']

    # Calculate the probability for each action dimension
    exponent = -((actions.detach().cpu().squeeze() - mu) ** 2) / (2 * sigma ** 2)
    probability = torch.exp(exponent) / (torch.as_tensor(sigma) * torch.sqrt(torch.tensor(2 * math.pi)))

    probability = torch.as_tensor(probability)
    # Joint probability is the product of individual probabilities
    joint_prob = torch.prod(probability)

    return joint_prob


def sample_prompt_and_completions(dataset, sequence_length=20):

    trajectory = dataset[np.random.randint(len(dataset))]

    if len(trajectory["rewards"]) < sequence_length + 1:
        raise ValueError("Trajectory too short for the specified sequence length.")

    start_idx = np.random.randint(len(trajectory["rewards"]) - sequence_length - 1)
    states = trajectory["observations"][start_idx: start_idx + sequence_length]
    actions = trajectory["actions"][start_idx: start_idx + sequence_length]
    rewards = trajectory["rewards"][start_idx: start_idx + sequence_length]
    time_steps = np.arange(start_idx, start_idx + sequence_length)

    # Use the final state of the prompt to select two completions
    final_state = trajectory["observations"][start_idx + sequence_length]

    # Identify candidate actions and rewards for the final state
    candidate_actions = trajectory["actions"][start_idx + sequence_length: start_idx + sequence_length + 2]
    candidate_rewards = trajectory["rewards"][start_idx + sequence_length: start_idx + sequence_length + 2]

    # Choose positive and negative actions based on rewards
    if candidate_rewards[0] > candidate_rewards[1]:
        positive_action = candidate_actions[0]
        negative_action = candidate_actions[1]
    else:
        positive_action = candidate_actions[1]
        negative_action = candidate_actions[0]

    return (
        torch.tensor(states),
        torch.tensor(actions),
        torch.tensor(rewards),
        torch.tensor(time_steps),
        torch.tensor(positive_action),
        torch.tensor(negative_action),
    )
...
def train(config: TrainConfig):
    ...
    ref_model = copy.deepcopy(model).to(config.device)
    train_model = copy.deepcopy(model).to(config.device)
    ...
    for epoch in trange(1000, desc='Fine Tuning'):
        total_loss = 0

        batch = sample_prompt_and_completions(dataset.dataset, config.seq_len)
        states, actions, rewards, timesteps, p_action, n_action = [b.to(config.device) for b in batch]

        p_actions = torch.cat((actions[:-1, :], p_action.unsqueeze(0)), dim=0)
        n_actions = torch.cat((actions[:-1, :], n_action.unsqueeze(0)), dim=0)

        model_prefered_logits = train_model(states=states, actions=p_actions, returns_to_go=rewards, time_steps=timesteps)[0, -1]
        model_disprefered_logits = train_model(states=states, actions=n_actions, returns_to_go=rewards, time_steps=timesteps,)[0, -1]
        ref_prefered_logits = ref_model(states=states, actions=p_actions, returns_to_go=rewards, time_steps=timesteps,)[0, -1]
        ref_disprefered_logits = ref_model(states=states, actions=n_actions, returns_to_go=rewards, time_steps=timesteps,)[0, -1]

        loss = calculate_DPO_loss(
            model_prefered_logits, model_disprefered_logits,
            ref_prefered_logits, ref_disprefered_logits,
            dataset.info, beta=0.1)
