...
class TrainConfig:
    ...
    num_workers: int = 4
    target_returns: Tuple[float, ...] = (3600.0, )
...
def eval_rollout(
    ...
    for step in range(model.episode_len):
        ...
        if (step + 1) % model.seq_len == 0:
            current_subgoal = predicted_subgoal
            slices = torch.as_tensor([-1] * config.subgoal_interval, dtype=torch.long)
            sg_states[:, : step + 1][:, -config.subgoal_interval:] = states[:, : step + 1][:, slices]
